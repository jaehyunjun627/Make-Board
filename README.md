...
엑셀에서 바로 열 수 있는 **`result.csv`** 로 저장하는 파이썬 프로그램입니다.
...
`result.csv` 는 항상 아래 8개 컬럼으로 만들어집니다.
...
수집 120건 중 고유 118건 → 'result.csv' 저장 완료 (기간: 2026-01-03 ~ 2026-08-09)
```
...
**Q. 엑셀에서 한글이 깨져요.**
`result.csv` 는 `utf-8-sig` 로 저장되므로 보통은 안 깨집니다. 그래도 깨지면
